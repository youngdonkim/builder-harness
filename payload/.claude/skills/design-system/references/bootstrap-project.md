...
- 처음부터 색인 형식으로 시작한다 — `이름 | 범용/전용 | 한 줄 설명 | 파일 경로 · 클래스`
- 이사 온 범용 컴포넌트 등록 + 앞으로 생길 전용 컴포넌트는 만들 때마다 등록
- 자세한 설명은 인벤토리가 아니라 **부품 파일 머리 주석**에 쓴다 — 그러니 부품을 만들 때 머리 주석부터 쓴다
- 흩어진 걸 모은 집계·여러 파일에 걸친 절차·코드에 없는 규칙(금지·용어 통제·사고 이력)은 인벤토리 항목이 아니라 문서의 별도 "함정" 절에 모은다
