...
- **main이면 멈춘다.** 여기서 동기화를 진행하지 않고, `/new-task`로 작업 브랜치를 먼저 만든 뒤 다시 "하네스 동기화해줘"를 불러달라고 안내한다. 이유는 짧게 짚어준다 — `auto-wip-commit`(턴이 끝날 때마다 바뀐 파일을 알아서 커밋해두는 훅)은 작업 브랜치에서만 돈다. main에서 그대로 동기화하면 바뀐 파일이 커밋 안 된 채 쌓여 되돌릴 지점이 안 남고, 나중에 `no-main-push` 훅에 막혀 올리지도 못한다.
  - **예외는 하나** — 하네스를 막 적용한 직후처럼 작업 브랜치를 아직 못 만드는 상황이다. 이때만, 사용자가 사정을 알고도 그대로 진행하겠다고 명시적으로 말하면 진행한다. 사용자가 아무 말 없으면 진행하지 않는다.
...
- **main에서 그대로 진행했으면**(1단계 예외에 걸려 사용자가 명시적으로 진행을 골랐던 경우 — 하네스를 막 적용한 직후라 작업 브랜치를 아직 못 만들던 때) — 지금 바뀐 파일이 아직 커밋 안 된 채 main에 남아 있다고 짚어주고, 작업 브랜치를 만들어 그 위에 커밋한 뒤 `/done-task`로 PR을 올리라고 안내한다.
