...
- **소프트 (막지 않고 ⚠ 보고)**: 원격이 이미 있는데 gh 활성 계정이나 이 저장소의 커밋 작성자가 표의 소유자와 다르면, 진행하되 §4 완료 보고에 한 줄 남긴다 — "⚠ 표의 소유자와 다른 계정(<계정>)으로 작업 중이야. 팀원이면 정상이고, 소유자 본인이면 계정을 확인해." 팀원은 자기 계정으로 일하는 게 정상이라 하드로 막으면 팀원의 모든 ship이 오탐으로 멈추고, 잘못된 계정의 push는 어차피 GitHub이 권한 오류로 거부한다 (§3-a가 처리).
