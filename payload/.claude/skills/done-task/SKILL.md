...
**1-0. 계정 검문** (프로젝트 `AGENTS.md`에 「소유와 계정」 표가 있을 때만 — 없으면 건너뛴다) — push는 원격 저장소가 계정에 묶이는 순간이다. gh 활성 계정·이 저장소의 커밋 작성자·원격 주소(SSH 호스트 별칭 포함)가 표의 소유자 것인지 대조하고, **다르면 push하지 않고 [결정 필요]로 멈춘다.** 원격이 아직 없으면 표의 계정 아래 만든다. (근거: 첫 push 때 gh에 로그인된 개인 계정이 조용히 회사 프로젝트 저장소의 소유자가 된 실사고 — CLI 활성 계정은 전역이라 다른 소유의 프로젝트를 오가면 바뀌어 있다.)
