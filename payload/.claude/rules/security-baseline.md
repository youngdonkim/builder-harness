...
역할 표·판정 함수를 실제로 만드는 절차는 `idea-to-mvp` 스킬의 `6-2-backend-auth.md` §11에 있다.
...
### 하지 말 것 아홉 **[①~⑧ 공식이 명시적으로 금지 · ⑨ 공식 권장 전환 + 실측 1건]**
...
| ⑨ | 서버(특히 미들웨어)에서 매 요청 `getUser()`로 세션 확인 | 모든 페이지 이동(`.rsc` 요청 포함)에 auth 서버 왕복이 얹힌다 — `getClaims()`로 로컬 서명 검증을 쓴다 |

**⑨만 등급이 다르다 [공식 권장 전환 + 실측 1건]** — 공식이 금지한 게 아니라 Supabase 공식 Next.js SSR 가이드가 `getUser` 대신 `getClaims` 권장으로 바뀐 것이다(supabase/supabase#39947). 실측: 로컬 요청당 +22.7ms, 실서비스에서 리전이 어긋난 경우 왕복 ~190ms.

- **전제 조건** — 프로젝트가 비대칭 JWT Signing Keys를 써야 한다. 2025-05 이후 만든 프로젝트는 기본이고, JWKS 엔드포인트에서 ES256 키가 보이면 된 것이다.
- **③과 층이 다르다** — ③은 "`getSession()`은 위조될 수 있으니 인가 근거로 쓰지 마라"(믿을 수 없다), ⑨는 "`getUser()`는 믿을 수 있지만 매 요청 부르면 비싸다"(느리다). 둘 다 답은 `getClaims()`다.
- **④의 주석 관행은 그대로다** — "미들웨어의 이 호출이 만료 토큰을 갱신한다 — 지우면 로그인이 조용히 풀린다" 주석은 `getClaims()`로 바꿔도 지우지 않는다.
