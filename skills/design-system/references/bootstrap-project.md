...
이 스킬은 builder-harness 플러그인에 들어 있어서, 플러그인을 설치한 프로젝트에서는 따로
복사할 필요 없이 바로 쓸 수 있다. 새 프로젝트에 이 디자인 시스템을 도입할 때는 아래
순서를 따른다.
외부 목업에서 출발한다면 0부터, 백지에서 출발한다면 1부터.
...
**이렇게 들어온 목업인지 확인하기**: 예를 들어 Claude Design에서 "Use a template →
Mobile app design"으로 만든 목업은 아래 무대 장치를 달고 온다. 이 도구 하나에 국한된 문제가
아니다 — 다른 목업 생성 도구도 종류만 다를 뿐 같은 성격의 장치를 남긴다. 마크업에 기기 프레임
...
- **번들 자산**: 목업에 통째로 박힌 폰트(수 MB)·이미지를 CDN 또는 최적화된 경로로 옮긴다.
