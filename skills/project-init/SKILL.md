...
- **다름** → 무단 덮어쓰기 금지. 어떤 파일이 어떻게 달라졌는지 변경 요지를 한 줄씩 모아 보여주고, 아래 세 선택지를 주고 사용자에게 확인한다 — 프로젝트에서 의도적으로 커스텀했을 수 있어서다.
  - **(a) 템플릿으로 교체** — 커스텀을 버리고 최신 템플릿 내용으로 덮어쓴다.
  - **(b) 커스텀 유지** — 그대로 두고 마무리 보고에 남긴다.
  - **(c) 병합** — 커스텀 내용은 그대로 두고 템플릿에서 새로 생기거나 바뀐 대목만 합친 병합안을 만들어 보여주고, 사용자가 확인하면 반영한다. 커스텀도 템플릿도 둘 다 살리고 싶을 때 쓴다.

  병합에는 안전 규칙이 하나 있다 — **같은 대목을 템플릿과 커스텀이 서로 다르게 고친 진짜 충돌은 AI가 임의로 정하지 않는다.** 그 대목만 양쪽 내용을 나란히 짚어 어느 쪽으로 갈지 사용자에게 물어보고, 나머지 안 겹치는 부분은 그대로 합친다.
...
다음 다섯 갈래로 요약해 보고한다: 갱신됨 / 새로 복사됨 / 병합됨 / 커스텀 유지됨 / 전부 최신.
...
- ❌ 동기화 모드에서 diff 없이 일괄 덮어쓰기 — 다른 파일은 반드시 사용자 확인 후 교체·병합
