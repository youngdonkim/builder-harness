...
3. **`screen-design.md` 머리에 `# 출발점 정리` 섹션을 적는다** (SKILL.md §2.2). 이전 단계까지 정리된 내용이 이 단계의 출발점임을 문서에 남기는 것이다:
...
- [ ] **출발점 정리** — `변경 없음` 한 줄 또는 4개 전문 재기술 + `## 변경` (SKILL.md §2.2).
...
2. **최신 출발점 정리** — `변경 없음`이면 `market-research.md` §최종 서비스 기획 요약 내용이 원문 (SKILL.md §2.2).
