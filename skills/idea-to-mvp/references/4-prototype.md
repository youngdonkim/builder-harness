...
3. 그 reference HTML을 **프로젝트 루트**에 `[projectName]-proto.html`로 사본 저장 (§2.3 — 테스트·단계 판별용 신호 파일).
4. 사용자가 브라우저로 그 파일(프로젝트 루트 사본)을 **더블클릭으로 열어** 시각·플로우 **직접 클릭 검증** (서버·빌드 불필요).
5. 피드백 받아 — 큰 변경이면 §2.2-A(Claude Design 재호출), 자잘한 예외면 §2.2-B(in-place 보정).
...
| **C. 같은 대화에서 누적된 조정만 다시 받고 싶을 때** (Claude Design과 티키타카로 여러 번 다듬었는데 zip 통째로 재호출은 부담) | 그 대화창에 **"가장 최신으로 standalone.html 파일을 생성해줘"** 프롬프트 → 그 시점까지 반영된 최신 standalone HTML(= reference HTML과 동일한 React+Babel 번들) 하나가 생성됨 → 다운로드해 현재 `prototype-ver-{N}/`의 reference HTML만 교체. A(새 zip 전체)보다 가볍고, B(직접 손 편집)보다 안전 — 번들을 직접 째지 않아도 됨. |

C가 있어도 A·B가 없어지는 건 아니다 — 구조가 통째로 바뀌는 큰 변경은 여전히 A(재호출 → 새 zip)가 기본, C는 *같은 대화 세션 안에서 쌓인 자잘한 반영분을 파일 하나로 회수*하는 용도.

**받은 뒤가 끝이 아니다 — 루트 사본 갱신 → 테스트 → 다음 단계 제안까지 이어간다.** A·B·C 어느 경로든 파일을 받아 `prototype-ver-{N}/`에 넣었으면, 거기서 멈추지 말고: (1) §2.3대로 **프로젝트 루트의 `[projectName]-proto.html` 사본도 최신으로 갱신** → (2) 사용자에게 **브라우저로 열어 직접 클릭 테스트**해보라고 안내 → (3) 테스트 결과가 괜찮고 §3 완료 체크리스트가 충족되면 **AI가 먼저** "이제 5단계(ProtoRetro — 데모 후 인터뷰)로 넘어가자"고 제안한다. 사용자가 알아서 다음 단계를 요청하길 기다리지 않는다 (SKILL.md §1.3 "AI가 먼저 제안" 원칙).

**SoT 판정**: 가장 큰 N의 `prototype-ver-{N}/` 폴더가 현재 SoT. 진입점은 그 안 reference HTML, 설계도는 README. 이전 ver 폴더는 history.

### 2.3 프로젝트 루트 신호 파일 — `[projectName]-proto.html`

A·B·C 어느 경로로 반영됐든, 최신 reference/standalone HTML은 `prototype-ver-{N}/` 저장과 별개로 **프로젝트 루트**(이 저장소 최상위 폴더 — `planning/` 밑이 아니라 `CLAUDE.md`가 바로 놓이는 위치)에도 사본을 하나 둔다.

- **파일명 규칙**: `[projectName]-proto.html` — 여기서 **`projectName`은 프로젝트 폴더명**이다 (이 저장소 최상위 디렉토리 이름 그대로. 예: 저장소 폴더가 `zipnavi`면 `zipnavi-proto.html`).
- **받아오는 법**: Claude Design에서 다운로드할 때 이 이름으로 바로 저장하거나, 기본 파일명으로 받은 뒤 프로젝트 루트로 옮기면서 이 이름으로 바꾼다.
- **갱신 시점**: ver-1 최초 수령 시 + 이후 §2.2 A(새 zip)·B(in-place 보정)·C(standalone 재생성) 중 어느 경로로 갱신되든 **매번 이 루트 사본을 같은 파일명으로 덮어쓴다** — 버전 번호를 붙이지 않아 항상 "지금 테스트해야 할 최신본" 하나만 프로젝트 루트에 존재한다.
- **SoT 아님**: 이 루트 사본은 편의·신호용 복사본일 뿐, SoT는 여전히 `prototype-ver-{N}/` 폴더 전체(README·토큰·컴포넌트까지 있어야 MvpBuild 입력 역할을 함). 루트 사본이 빠졌거나 오래돼도 SoT 무결성엔 영향 없다 — 그냥 다시 복사하면 복구된다.
- **왜 두나 — 신호로 쓰기 위해서**: 이 파일이 프로젝트 루트에 있다는 사실 자체가 "프로토타입 빌드 라운드는 끝났고 지금은 테스트 단계"라는 신호다. 사람은 폴더 깊이 안 들어가고 바로 더블클릭해서 열 수 있고, AI는 `/idea-to-mvp` 재호출 시 이 파일 존재 여부로 지금이 막 프로토타입이 나와 테스트 라운드에 들어갈 시점인지 빠르게 판단한다 (SKILL.md §1.2 현재 단계 판별에서 함께 확인).
...
- [ ] **프로젝트 루트에 `[projectName]-proto.html`이 최신 상태로 있음** (§2.3 신호 파일 — projectName은 프로젝트 폴더명).
- [ ] (사용자 검증) 사람이 브라우저로 reference HTML(또는 프로젝트 루트 사본)을 클릭하며 검토 완료.

체크리스트 미충족 시 SKILL.md §4 단계 진입 평가 정책에 따라 사용자에게 선택지 제시.

## 4. 산출물 스펙

위치: `planning/mvp/prototype-ver-{N}/` (버전별 폴더, 단일 `prototype.md` 파일 없음). Claude Design zip을 **압축 해제한 구조 그대로** 보관. 이와 별개로 **프로젝트 루트**에 신호 파일 사본 하나(§2.3)를 둔다.

```
(프로젝트 루트)
├── CLAUDE.md
├── [projectName]-proto.html       ← §2.3 신호 파일 (projectName = 프로젝트 폴더명, 항상 최신본 하나만)
└── planning/mvp/
    ├── market-research.md
    ├── screen-design.md
...
- **`[projectName]-proto.html`은 SoT가 아니라 프로젝트 루트의 신호·편의 파일** — 상세는 §2.3.
...
- **standalone.html(§2.2-C) 받아 교체한 뒤**: "`prototype-ver-{N}/`에 최신 반영본 넣었어 — 브라우저로 열어서 한 번 쭉 클릭해봐. 괜찮으면 이제 진짜 사람들한테 보여주고 반응 볼 차례야."
