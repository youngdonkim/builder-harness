...
**받은 뒤가 끝이 아니다 — 테스트 → 다음 단계 제안까지 이어간다.** A·B·C 어느 경로든 파일을 받아 `prototype-ver-{N}/`에 넣었으면, 거기서 멈추지 말고: (1) 사용자에게 **브라우저로 열어 직접 클릭 테스트**해보라고 안내 → (2) 테스트 결과가 괜찮고 §3 완료 체크리스트가 충족되면 **AI가 먼저** "이제 5단계(ProtoRetro — 데모 후 인터뷰)로 넘어가자"고 제안한다. 사용자가 알아서 다음 단계를 요청하길 기다리지 않는다 (SKILL.md §1.3 "AI가 먼저 제안" 원칙).
