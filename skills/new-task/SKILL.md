---
name: new-task
description: PR 머지 후 작업 단위 전환 — origin/main 싱크 + 다 끝난 옛 작업 폴더(워크트리)·브랜치 정리(local·remote) + 새 작업 폴더와 브랜치 자동 생성. 사용자 의도(args 또는 follow-up)에서 type·topic을 Claude가 추론. "다음 작업 시작", "새 작업 폴더 만들어줘", "새 브랜치 만들어줘", "PR 머지했어 다음 가자" 등에 사용.
