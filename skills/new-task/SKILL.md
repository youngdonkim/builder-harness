...
**세션 이동은 2단이다.** fork로 도는 이 스킬은 워크트리를 만들고 경로를 보고할 뿐이고, 세션을 그 폴더로 옮기는 `EnterWorktree` 호출은 **메인 세션이** 한다 (서브에이전트가 호출해봐야 부모 세션은 안 따라온다). 그래서 완료 보고 맨 끝에 "메인 세션이 할 일"을 반드시 넣는다 (§5). 이게 빠지면 세션이 옛 폴더에 그대로 남는다.
