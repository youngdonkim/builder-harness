...
머지가 확인되면 **원격 브랜치가 아직 살아있는지 보고 지운다.** 저장소 설정이 "머지된 브랜치 자동 삭제"면 이미 사라져 있으니, 있을 때만 지운다 (없는 브랜치를 지우려 하면 오류가 난다):

```bash
git ls-remote --heads origin <branch>        # 출력이 있으면 아직 살아있다
git push origin --delete <branch>            # 살아있을 때만
```

로컬 브랜치와 작업 폴더는 여기서 안 건드린다 — 지금 세션이 그 안에 앉아 있어서 지우면 발밑이 사라진다. 정리는 `new-task` 몫이다 (§안 하는 것).
...
✓ remote의 <branch> 삭제됨 [워크트리에서 §3-d 오류를 만났으면 한 줄 덧붙임: "○ gh가 로컬 브랜치까지 치우려다 'main is already used by worktree' 오류를 냈는데, 머지 자체는 성공이라 그대로 진행했어 — 워크트리에선 정상이야"]
...
머지되고 나면 로컬 정리는 *이 폴더에서* /new-task로 하면 돼 (main 싱크 + 이 브랜치 정리 + 새 브랜치).
/new-task는 그때 앉아 있는 폴더만 정리해서, 다른 폴더로 옮긴 뒤엔 이 폴더가 알아서 치워지지 않아.
```
...
| 상황                                                    | 처리                                                                     |
| ------------------------------------------------------- | ------------------------------------------------------------------------ |
| 워크트리에서 실행됨                                     | 정상. push·PR·머지 모두 그대로 작동. 워크트리는 남겨둠. §3-d는 `--delete-branch` 빼고 머지 후 원격 브랜치를 따로 지움 |
| `gh pr merge`가 `'main' is already used by worktree`로 실패 | 머지는 이미 성공한 상태다. `gh pr view --json state`가 `MERGED`면 그대로 진행 (§3-d) |
