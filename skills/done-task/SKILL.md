...
- 성공 시 다음 단계로.
- 머지 실패 (conflict·branch protection·CI 미통과·**권한 부족** 등) → 중단 + 사용자에게 stdout + PR URL 보고: "GitHub에서 직접 해결한 다음 다시 호출해줘." (권한 부족으로 실패한 거면 애초에 팀원 경로였어야 하는 상황이니, PR은 그대로 두고 팀원용 보고에 준해 안내)
