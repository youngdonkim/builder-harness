...
| typography/family | 글꼴 자체 | `--font-family-base`(본문 기본) · 필요시 `--font-family-brand`·`--font-family-mono` | `--font-family-{역할}` — 값은 foundation의 폰트 변수를 가리키는 별칭 |
...
`2-mockup.md` — 목업 단계 실사용 검증에서 나온 요건이지만, 크기·굵기·대비는 특정 단계가 아니라
...
역할 한정 토큰은 **어디 전용인지, 다른 자리에 쓰면 왜 안 되는지**까지 적는다 — 예: `/* 캡션 전용 — 본문 바닥선 아래 값 */`.
