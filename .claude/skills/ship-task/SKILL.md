...
# gh 호출은 §1-0의 토큰 로더를 같은 셸 호출 앞에 단다: env GH_TOKEN="$(sed -n 's/^GH_TOKEN=//p' .env.cli 2>/dev/null)"
...
```
[§1-0 계정 검문에서 남길 줄이 있으면 여기에 (없으면 통째로 생략):
  "○ 이 프로젝트는 아직 gh 토큰 방식이 아니야 — `.env.cli`에 `GH_TOKEN`을 넣어 옮기는 게 좋아
   (`docs/account-check.md` gh 항목)" / "○ gh 활성 계정을 <계정>으로 전환하고 진행했어"
  / "⚠ `.env.cli`의 `GH_TOKEN`이 표와 다른 계정(<계정>) 것이야" / "⚠ 표의 소유자와 다른 계정(<계정>)으로 작업 중이야"]
