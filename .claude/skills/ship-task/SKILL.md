...
allowed-tools: Bash(git *) Bash(gh *) Bash(npm run smoke)
context: fork
agent: git-flow
---
...
메인 세션의 `done-task` 스킬이 부르는 내부 스킬이다 — 사람이 직접 칠 일은 없다. 현재 브랜치의 WIP 커밋들을 PR 흐름으로 main에 ship한다. **(코드 변경 시 simplify 게이트) → (조건부 origin/main 합치기) → (마이그레이션 있을 때만 마이그레이션 게이트) → (`smoke` 스크립트 있을 때만 스모크 게이트) → push → PR 생성 → (머지 권한 있을 때만) CI 대기 → squash merge → remote 브랜치 삭제** 한 흐름.
...
**호출 경로는 사용자 ship 신호 → done-task → 이 스킬이다.** 처음 방아쇠는 사용자 입에서 나와야 한다 — "보내줘"·"머지해줘"·"올려줘", 또는 클로드가 "화면 확인했으면 보낼까?"라고 물었을 때의 "ㅇㅇ" 같은 답. ship은 main 머지고, main 머지는 (Vercel이 붙어 있으면) 곧 실서비스 배포다. 그 한마디가 **사람이 화면을 직접 보고 괜찮다고 판단했다는 유일한 증거**라서다 (화면 검증은 사람 몫이라는 원칙 — 클로드는 화면을 볼 수 없다). 그 신호를 받은 메인 세션의 done-task가 simplify 판단을 끝낸 뒤 이 스킬을 부른다.

**호출 이후의 [결정 필요]는 메인 세션(done-task)이 판단한다.** simplify 스킵·커밋/stash·동기화 방식·마이그레이션 종류 같은 결정은 맥락을 더 많이 쥔 메인이 골라 args에 담아 이 스킬을 재호출한다. 이미 사용자가 ship을 승인한 뒤라 방아쇠는 이미 당겨져 있고, 남은 건 어떻게 보낼지의 판단이다. 다만 마이그레이션 종류(§1.7)처럼 사실 확인이 필요한 항목은 아는 게 없으면 지어내지 말고 사용자에게 물어라.
...
**이 스킬의 호출자는 메인 세션의 done-task 스킬이다** — 사용자의 ship 신호를 받은 done-task가 simplify 판단을 마치고 인자를 그대로 넘겨 부른다.

**사용자의 ship 신호 = 승인, 호출은 done-task가 대행**: 이 스킬이 도는 시점엔 사용자가 이미 ship(푸시·PR 생성·(머지 권한 있으면) 머지까지)을 승인한 상태다 — PR 제목·body는 AI 초안 그대로 사용, 별도 확인 없음 (사용자가 정한 규칙). 머지 권한이 없으면 애초에 승인해도 머지가 안 되니, 이 경우 승인의 범위는 자연히 PR 생성까지로 줄어든다 (§2.5).
...
**1-0. 계정 검문** (프로젝트 `AGENTS.md`에 「소유와 계정」 표가 있을 때만 — 없으면 건너뛴다) — 절차·서비스별 함정은 `docs/account-check.md`를 읽는다. 특히 원격 저장소가 아직 없어 첫 push로 생기는 경우가 이 문서의 검문 대상이다. 검사가 두 축이다.

- **하드 (다르면 멈춤)**: ① 원격 주소(SSH 호스트 별칭 포함)가 표 소유자의 저장소를 가리키는가. ② 원격이 아직 없어 새로 만들 때는, gh 활성 계정이 표의 GitHub 계정인가 — 원격 생성이 저장소가 계정에 묶이는 순간이라, 다르면 **만들지 않고 [결정 필요]로 멈춘다.** (근거: 첫 push 때 gh에 로그인된 개인 계정이 조용히 회사 프로젝트 저장소의 소유자가 된 실사고 — CLI 활성 계정은 전역이라 다른 소유의 프로젝트를 오가면 바뀌어 있다.)
- **소프트 (원격이 이미 있을 때)**: gh 활성 계정이 표와 다르고 **표의 계정이 이 컴퓨터에 등록돼 있으면, 경고에 그치지 말고 표의 계정으로 전환한 뒤 진행한다** — 로컬 전환이라 되돌리기 쉽고, §4 완료 보고에 전환 사실을 한 줄 남긴다. 표의 계정이 등록돼 있지 않거나(팀원이 자기 계정으로 일하는 정상 경우) 이 저장소의 커밋 작성자가 표와 다르면, 막지 않고 진행하되 §4 완료 보고에 한 줄 남긴다 — "⚠ 표의 소유자와 다른 계정(<계정>)으로 작업 중이야. 팀원이면 정상이고, 소유자 본인이면 계정을 확인해." 하드로 막으면 팀원의 모든 ship이 오탐으로 멈추고, 잘못된 계정의 push는 어차피 GitHub이 권한 오류로 거부한다 (§3-a가 처리).
...
- 비어 있지 않으면 args에 결정 지시(예: "변경은 커밋하고 진행")가 있으면 그대로 적용하고 계속. 없으면 **[결정 필요] 반환**:
  ...
  재호출 예시: `ship-task 변경은 커밋하고 진행` / `ship-task stash하고 진행`
  ```

### 1.5 simplify 게이트 (코드 변경이 있을 때만)

ship 전에 코드 품질 정리(`simplify` 스킬 — 재사용·단순화·효율·계층 리뷰 후 수정 적용)를 거쳤는지 확인하는 관문.
**정상 흐름에선 여기 안 걸린다** — 메인의 done-task가 위임 전에 simplify 판단을 끝내고 표식 커밋을 남긴다. 이 게이트는 그 판단이 빠졌을 때의 안전망이다.
...
- **simplify 표식 커밋이 없음** → **[결정 필요] 반환** (fork 안에서 simplify 스킬을 직접 실행하지 않는다 — 스킬 호출은 메인 세션 몫).
  ...
  선택지:
  ① 메인 세션(done-task)이 /simplify를 돌리고 표식 커밋을 남긴 뒤 ship-task 재호출.
     고친 게 있으면 `wip: /simplify …`, 고칠 게 없으면 `wip: /simplify — 변경 없음` 빈 커밋으로.
  ② 스킵하고 바로 ship
...
3. **이미 push한 브랜치라도 충돌이 있으면 멈춘다.** 합치기는 push된 브랜치에서도 되니 사용자가 실제로 고를 수 있는 행동이 있다. 그래도 그냥 올리고 싶으면 `동기화 없이 진행` 지시로 통과할 수 있다.
...
- **앞서 있음 + 충돌 있음** → **[결정 필요] 반환**:
  ...
  ① (권장) 로컬에서 합치기(merge)로 충돌 풀고 재호출 —
     `git merge origin/main` → 충돌난 파일 고치기 → `git add <파일>` → `git commit` → 다시 ship-task 호출
  ...
  ③ (최후 수단) 굳이 한 줄로 깔끔한 기록(선형 기록)을 원하면 로컬에서 직접 리베이스 —
     `git rebase origin/main` → 충돌난 파일 고치기 → `git add <파일>` → `git rebase --continue` → 다시 ship-task 호출
  ...
  재호출 예시: `ship-task 머지로 진행` (① 선택 시) / `ship-task 동기화 없이 진행` (② 선택 시)
  ```
...
- **파일이 있고 결정 지시가 없음** → **[결정 필요] 반환** (미는 건 사람 몫이라 이 스킬은 supabase 명령을 직접 실행하지 않는다):
  ...
  재호출 예시: `ship-task 마이그레이션 밀었어 진행` / `ship-task 삭제형이라 머지 후에 밀 것 진행`
  ```

> 근거: CI의 밀림 감지는 main push **뒤에** 돈다 — 더하는 마이그레이션이면 이미 코드가 배포된 뒤에야 알려주는 셈이라, 깨진 걸 사후에 통보받는 꼴이다. 이 게이트는 **머지 전에** 잡으니 애초에 안 깨진다. 둘은 겹치는 게 아니라 앞뒤로 붙는 이중 장치다 — ship-task 게이트(머지 전, 예방) → CI 감지(머지 후, 그물).

### 1.8 스모크 게이트 (`smoke` 스크립트가 있을 때만)

ship 전에 핵심 계약이 살아 있는지 스모크 테스트(smoke test — 핵심 동작만 빠르게 훑는 검사)로 확인하는 관문. §1.6 합치기 **뒤**에 두는 이유 — origin/main을 합친 최종 코드로 돌아야, main에 실릴 코드 그대로를 검사한다.

simplify 게이트(§1.5)와 달리 **fork 안에서 직접 실행한다** — simplify가 [결정 필요]로 미루는 건 스킬 호출이 메인 세션 몫이라서고, 스모크는 `npm run smoke` 단일 명령이라 마이그레이션 게이트처럼 fork가 스스로 판정까지 할 수 있다.

```
package.json에 "smoke" 스크립트가 있나? (package.json을 Read로 확인)
  없음 (파일 자체가 없어도 같은 취급) → 게이트 skip, 아무 말 없이 §2로
  있음 ↓
    args에 "스모크 스킵" 류 지시 있음 → 게이트 통과, §2로
    npm run smoke 실행
      성공 → §2로. §4 완료 보고에 "스모크 N건 통과" 한 줄
      실패 → [결정 필요]로 멈춤
```

- **`smoke` 스크립트 없음** (package.json이 없거나, 있어도 `scripts`에 `smoke`가 없음) → 게이트 skip. 스모크가 없는 프로젝트(6단계 이전)가 대부분이라 여기서 멈추면 안 된다. 아무 말 없이 §2로 간다.
- **args에 "스모크 스킵" 류 지시 있음** → 게이트 통과, §2로 (해석은 §호출 패턴).
- **`npm run smoke` 성공** → §2로. 출력에서 통과 건수를 읽어 §4 완료 보고에 "스모크 N건 통과" 한 줄을 남긴다.
- **실패** → **[결정 필요] 반환**:
  ```
  [결정 필요] 스모크 테스트가 실패했어. 이대로 ship하면 깨진 상태가 main에 실려.

  실패 출력 요약:
  <실패한 검사 이름·오류 메시지 요약>

  선택지:
  ① (권장) 실패 원인 고치고 재호출 — 다시 ship-task를 부르면 스모크가 또 돌아
  ② 스킵하고 그대로 ship — 실패가 코드 문제가 아니라 스모크 쪽 문제(환경·테스트 자체 결함)라고 확인했을 때만

  재호출 예시: `ship-task 스모크 스킵` (② 선택 시)
  ```

> 근거: 6-3-backend-deploy §2.3이 스모크 테스트를 만들게 하지만 ship 전 실행을 강제하는 장치가 없어, 깨진 계약이 그대로 main에 실려 배포된 사고가 있었다. 이 게이트가 push 전에 잡는다.
...
- **실패** → 머지하지 않고 중단. 실패한 검사 이름과 PR URL을 사용자에게 보고하고, "고쳐서 push하면 CI가 다시 돌아, 그 뒤 `/done-task` 재호출"이라고 안내한다. PR은 지우지 않고 그대로 둔다. **실패 원인이 배포 플랫폼의 Git 계정 연결이면 코드 문제가 아니다** — 커밋 작성자의 Git 계정이 소유자의 배포 플랫폼 계정에 연결돼 있는지 확인해 복구하라고 안내한다. 복구 전에는 머지하지 않는다(실서비스 배포도 같은 이유로 막힌다) — 복구한 뒤 검사를 다시 돌린다.
...
- **`new-task`의 `git remote prune origin`과 역할이 겹치지 않는다**: **ship-task는 자기가 방금 만든 잔재를 치우고, new-task는 그 외 전부를 훑는다.** 팀원 경로에선 자기가 머지하지 않아 ship-task가 여기까지 오지 않고, 다른 사람이 지운 브랜치의 잔재도 여기선 안 걷힌다 — 그건 new-task가 시작할 때 훑어서 정리한다. 그래서 `new-task` 쪽은 그대로 둔다.
...
✓ [§1.8 스모크를 돌렸으면 한 줄: "스모크 <N>건 통과". `smoke` 스크립트가 없어 안 돌았으면 이 줄 생략.
  "스모크 스킵" 지시로 건너뛰었으면 대신: "⚠️ 스모크 실패(또는 실행)를 지시대로 스킵하고 올렸어"]
...
(§1.6 충돌 [결정 필요]를 `동기화 없이 진행` 지시로 통과시킨 경우, 판정 결과 줄을 대신 이렇게 쓴다:
...
| package.json에 `smoke` 스크립트 없음 (파일 자체가 없는 경우 포함) | 게이트 skip — 아무 말 없이 다음 단계로. 스모크 없는 프로젝트가 대부분 (§1.8) |
| `npm run smoke` 실패                                    | [결정 필요]로 멈춤 — 실패 출력 요약 + 고치고 재호출 / 스킵 선택지 제시 (§1.8) |
| 스모크를 "스모크 스킵" 지시로 통과                      | 그대로 ship하되, 완료 보고에 스킵 사실을 ⚠️로 남김 (§1.8, §4)             |
...
아래 인자는 메인의 done-task가 사용자 의도와 [결정 필요] 결정을 담아 넘긴다.

- `ship-task` — args 없음. PR title·body는 자동 추출(branch명·diff·wip 메시지)만으로 초안 짜서 바로 ship.
- `ship-task <자연어 의도>` — 자연어를 *사용자의 핵심 강조 hint*로 받음. Claude가 자동 추출 + hint 종합해 초안 짬. hint가 _title 그 자체가 되는 건 아님_ — 다른 변경도 함께 묶어서 종합 title 만듦. 예: `ship-task 다크모드 토글 추가` → "다크모드 토글 + 색 토큰 정리" 식으로 종합.
- `ship-task 스킵` — §1.5 simplify 게이트 [결정 필요]에 대한 결정. 코드 변경 있어도 simplify 없이 바로 ship.
- `ship-task 변경은 커밋하고 진행` — §1-c working tree [결정 필요]에 대한 결정을 담아 재호출.
- `ship-task 머지로 진행` — §1.6 충돌 [결정 필요]에 대한 결정(선택지 ①). 사용자가 로컬에서 `git merge origin/main`으로 충돌을 이미 풀었을 때 씀. 이 경우 내 브랜치가 origin/main을 이미 품고 있어서 §1.6 판정이 "앞서 있지 않음"으로 나와 자연히 통과한다.
- `ship-task 동기화 없이 진행` — §1.6 충돌 [결정 필요]에 대한 결정(선택지 ②). origin/main과 맞추지 않고 그냥 PR만 올림. **옛 표현 `리베이스 없이 진행`도 같은 뜻으로 받는다** (판정이 리베이스에서 합치기로 바뀌기 전 이름).
- `ship-task 마이그레이션 밀었어 진행` — §1.7 마이그레이션 게이트 [결정 필요]에 대한 결정. **"밀었다" 취지**(예: "밀었어", "db push 했어", "먼저 밀고 왔어")면 게이트를 그대로 통과시키고 §2로 간다. 추가로 챙길 건 없다.
- `ship-task 삭제형이라 머지 후에 밀 것 진행` — §1.7 마이그레이션 게이트 [결정 필요]에 대한 결정. **"없애는 거라 머지 후에 민다" 취지**(예: "삭제형이야", "머지 후에 밀게")면 통과시키되, **머지 후에 밀라는 안내를 §4 완료 보고에 반드시 남긴다는 표시를 들고 간다.**
- `ship-task 스모크 스킵` — §1.8 스모크 게이트 [결정 필요]에 대한 결정. 스모크가 실패해도(또는 아예 안 돌리고) 그대로 ship한다. 이 경우 §4 완료 보고에 스킵 사실을 ⚠️로 남긴다.
- `ship-task 머지까지` — §2.5 권한 판정과 상관없이 오너/메인테이너 경로로 진행 (권한 조회가 실패했지만 실제로 머지 권한이 있는 걸 알 때 씀).
- `ship-task 머지하지 마` / `ship-task PR까지만` — §2.5 권한 판정과 상관없이 팀원 경로로 진행 (오너라도 이번엔 리뷰부터 받고 싶을 때 씀).

## 안 하는 것 (의도적)

- ❌ 다른(옛) 브랜치 정리·새 브랜치 생성 — 그건 `new-task` 스킬 몫. 이번에 머지한 브랜치의 뒷정리(원격·로컬 삭제 + main 최신화)는 §3-d에서 직접 끝낸다
- ❌ done-task를 거치지 않은 호출 — 이 스킬은 사용자 ship 신호를 받은 메인의 done-task만 부른다. 처음 방아쇠는 사람 몫이다. main 머지 = 실서비스 배포라, "사람이 화면을 봤다"는 증거가 사용자의 그 한마디뿐이다
- ❌ stage·commit 자동화 — *현재 commit된 wip*만 ship. 추가 변경은 사용자가 commit 또는 §1-c (a) 옵션 선택.
- ❌ simplify 스킬 직접 실행 — fork 안에는 Skill 도구가 없다. §1.5에서 [결정 필요] 반환만 하고, 실제 `/simplify` 실행과 표식 커밋은 메인 세션(done-task) 몫이다
...
- ❌ 스모크 없는 프로젝트에서 스모크 만들라고 요구 — `smoke` 스크립트가 없으면(6단계 이전 프로젝트) 아무 말 없이 통과한다. 스모크를 만드는 건 6-3-backend-deploy 단계 몫 (§1.8)
- ❌ 스모크 실패를 조용히 넘기기 — `npm run smoke`가 실패하면 반드시 [결정 필요]로 멈추고, 스킵은 명시 지시(`스모크 스킵`)로만 통과한다 (§1.8)
- ❌ 다른 base 브랜치 — main 전제
