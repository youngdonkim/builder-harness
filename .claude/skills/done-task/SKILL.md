...
> 근거: 6-backend-build §2.4가 스모크 테스트를 만들게 하지만 done-task 전 실행을 강제하는 장치가 없어, 깨진 계약이 그대로 main에 실려 배포된 사고가 있었다. 이 게이트가 push 전에 잡는다.
...
- ❌ 스모크 없는 프로젝트에서 스모크 만들라고 요구 — `smoke` 스크립트가 없으면(6단계 이전 프로젝트) 아무 말 없이 통과한다. 스모크를 만드는 건 6-backend-build 단계 몫 (§1.8)
