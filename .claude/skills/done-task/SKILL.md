...
현재 브랜치의 WIP 커밋들을 PR 흐름으로 main에 ship하는 스킬. **(코드 변경 시 simplify 게이트) → (조건부 origin/main 합치기) → (마이그레이션 있을 때만 마이그레이션 게이트) → (`smoke` 스크립트 있을 때만 스모크 게이트) → push → PR 생성 → (머지 권한 있을 때만) CI 대기 → squash merge → remote 브랜치 삭제** 한 흐름.
...
- **실패** → **[결정 필요] 반환**:
  ...
  선택지:
  ① (권장) 실패 원인 고치고 재호출 — 다시 /done-task 부르면 스모크가 또 돌아
  ② 스킵하고 그대로 ship — 실패가 코드 문제가 아니라 스모크 쪽 문제(환경·테스트 자체 결함)라고 확인했을 때만

  재호출 예시: `/done-task 스모크 스킵` (② 선택 시)
  ```

> 근거: 6-3-backend-deploy §2.3이 스모크 테스트를 만들게 하지만 done-task 전 실행을 강제하는 장치가 없어, 깨진 계약이 그대로 main에 실려 배포된 사고가 있었다. 이 게이트가 push 전에 잡는다.
...
- `/done-task 스모크 스킵` — §1.8 스모크 게이트 [결정 필요]에 대한 결정. 스모크가 실패해도(또는 아예 안 돌리고) 그대로 ship한다. 이 경우 §4 완료 보고에 스킵 사실을 ⚠️로 남긴다.
