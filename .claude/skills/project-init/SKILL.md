...
앞의 두 답은 idea-to-mvp 1단계(UserStory)의 필수 입력 1·2와 같은 항목이다 — AGENTS.md에 박혀 1단계가 다시 묻지 않는다. 소유 답은 [3단계](#3-agentsmd-생성)에서 `AGENTS.md`의 「소유와 계정」 표를 채우는 데 쓴다.

### 2. CLAUDE.md 생성

프로젝트 루트에 `CLAUDE.md`를 만들고 **내용을 정확히 `@AGENTS.md` 한 줄로** 쓴다. 템플릿은 없다 — 이 파일은 규칙을 담지 않고, 클로드 코드가 규칙 정본인 `AGENTS.md`를 읽게 하는 포인터일 뿐이다. 이미 있고 내용이 그 한 줄이면 그대로 둔다.

*왜 한 줄만 두나*: 클로드 코드는 버전·실행 환경에 따라 `AGENTS.md`를 혼자 읽지 못한다. 그래서 어디서 열어도 규칙이 읽히도록 이 포인터를 남기되, 규칙 자체는 한 파일에만 둔다.

### 3. AGENTS.md 생성

`AGENTS.md`는 이 프로젝트의 **규칙 정본**이다. 클로드뿐 아니라 Codex 같은 다른 AI 도구도 이 파일을 읽는다.
...
- **gh 인증은 이 프로젝트의 발급 토큰으로 맞춘다** — 표의 GitHub 계정으로 개인 접근 토큰을 만들어 프로젝트 루트의 CLI 전용 깃 미추적 파일 `.env.cli`에 `GH_TOKEN=...` 한 줄로 두라고 사용자에게 안내하고, `.gitignore`에 `.env.cli`가 들어 있는지 확인해 없으면 넣는다. **전역 활성 계정 전환은 쓰지 않는다** — 전환은 컴퓨터 전체에 걸려 다른 소유의 프로젝트를 망가뜨리고, 토큰이 걸리면 효과도 없다. 아직 토큰이 없으면 활성 계정을 표와 맞춰 임시로 쓰되 토큰으로 옮기라고 안내한다. 계정 절차·서비스별 함정은 `payload/docs/account-check.md`(프로젝트에 복사되면 `docs/account-check.md`)에 있다.
...
- 2단계에서 만든 `CLAUDE.md`의 내용이 정확히 `@AGENTS.md`인지 확인한다.
...
**복사에서 빼는 파일 둘** — 미러 규칙의 예외는 이 둘뿐이다. 그대로 놓이는 파일이 아니라서 빠진다.

- `payload/AGENTS.md.template` — 복사 대상이 아니라 가공 대상. [3단계](#3-agentsmd-생성)에서 이미 썼다.
...
**payload 미러로 온 파일은 전부 하네스 소유물**이고, **`AGENTS.md` 하나만 예외**다. `CLAUDE.md`는 `@AGENTS.md` 한 줄짜리 포인터라 하네스 소유물로 다룬다 — 다만 옛 구조로 만든 프로젝트는 여기에 규칙이 들어 있을 수 있어, 확인 없이 줄이지 않는다([4. AGENTS.md — 예외 하나, 병합이 기본](#4-agentsmd--예외-하나-병합이-기본)).
...
### 4. AGENTS.md — 예외 하나, 병합이 기본

`AGENTS.md`는 프로젝트가 자기 사정에 맞게 채우고 덧붙이는 규칙 정본이라 덮어쓰지 않는다. 비교도 파일 전체가 아니라 **템플릿(`payload/AGENTS.md.template`)에서 온 부분만** 한다.

- **보존 대상 (대조 안 함)** — placeholder가 채워진 프로젝트 고유 내용(서비스 이름·한 줄 정의·타겟, 「소유와 계정」 표에 채워진 값)과 프로젝트가 스스로 추가한 문장·섹션(디자인 시스템 adapter에 채워진 경로, 앱 개발 컨벤션에 쌓인 규칙 등). 템플릿과 다른 게 정상이고, 병합해도 그대로 남는다.
- **대조 대상** — 템플릿에서 그대로 오는 공용 문구와 구조. 작업 원칙 섹션, 진행 방식·기획 문서 위치 안내문, 섹션 골격과 제목, 그 안에 박힌 단계 번호 같은 것. 신규 적용 모드에서 "수정하지 않고 그대로" 복사하는 부분([3. AGENTS.md 생성](#3-agentsmd-생성))이 곧 이 범위다.
- **대조·병합 범위는 `<!-- BEGIN:project-rules -->` 구역 안으로만 한정한다.** 마커 밖과 `<!-- BEGIN:nextjs-agent-rules -->`처럼 다른 도구가 만든 구역은 절대 건드리지 않는다.

*왜 대조하나*: 공용 문구는 하네스가 바뀌면 같이 낡는다. 예를 들어 `/idea-to-mvp`가 7단계로 개편되면 프로젝트 AGENTS.md에 남은 "6단계로 검증·개발한다"가 틀린 안내가 되는데, 파일 전체를 비교에서 빼두면 아무도 못 잡는다.
...
#### CLAUDE.md — 한 줄로 줄이되, 옛 내용은 확인받고 옮긴다

`CLAUDE.md`는 규칙을 담지 않는 포인터라 **내용이 정확히 `@AGENTS.md` 한 줄인지만 확인한다.** 그 한 줄이면 손대지 않고 넘어간다.

**그 밖의 내용이 들어 있으면 옛 구조로 만든 프로젝트다** — 예전에는 위임 원칙 같은 규칙이 이 파일에 있었다. 그 내용은 규칙이지 찌꺼기가 아니므로 **지우지 않는다.** 이행 절차는 이렇다:

1. `@AGENTS.md` 줄 말고 남아 있는 내용을 전부 뽑아, 무엇이 들어 있는지 사용자에게 **그대로 보여준다.**
2. 각 대목을 `AGENTS.md`의 어느 절로 옮길지 안(案)을 함께 보여준다. 이미 `AGENTS.md`에 같은 규칙이 있으면 "옮길 필요 없음(중복)"으로 표시하고, 없으면 옮길 자리를 짚는다.
3. **사용자 확인을 받은 뒤에** 그 내용을 `AGENTS.md`로 옮긴다 (위 병합 안전 규칙 둘이 그대로 적용된다).
4. 옮기기가 끝난 다음에야 `CLAUDE.md`를 `@AGENTS.md` 한 줄로 줄인다.

**확인 없이 지우거나 줄이지 않는다.** 사용자가 옮기지 말라고 하면 `CLAUDE.md`를 그대로 두고, 그 사실을 마무리 보고에 적는다.
...
- **CLAUDE.md** — `@AGENTS.md` 한 줄 그대로 / 옛 내용을 AGENTS.md로 옮기고 한 줄로 줄임 / 옛 내용을 그대로 둠(사용자가 고름)
