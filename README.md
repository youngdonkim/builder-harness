...
- **역할 B. 하네스 개발자** — 이 builder-harness 저장소 자체를 만들고 고치는 사람. 곧 이 저장소의 관리자이거나, 저장소를 자기 컴퓨터에 내려받아(clone) 수정한 뒤 PR(Pull Request — 반영해 달라는 제안)로 올리는 기여자다.

  → **저장소를 내려받아 둔 폴더의 경로**로 등록한다. 이렇게 하면 파일을 고칠 때마다 그 수정 내용이 **자기 컴퓨터의 Claude Code에서만** 곧바로 반영된다 — GitHub에 올리기 전에 고친 것이 잘 작동하는지 자기 환경에서 먼저 시험해보기 위한 등록 방식이다. 다른 사용자에게는 아무 영향이 없다.

  **다른 사용자에게 반영되는 정식 경로는 따로 있다**: 수정 → PR → 관리자가 main 브랜치에 합침(merge) → 각 사용자가 업데이트(아래 "업데이트 흐름"). 이 저장소는 main에 바로 올리는(push) 것이 규칙으로 막혀 있어, 관리자든 기여자든 PR을 거친다.
