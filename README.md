...
"어디 있는지"를 알려주는 방법이 두 가지 있다. **이 둘은 같은 걸 두 번 하는 게 아니라, 상황에 맞는 것 하나만 고르는 것**이다:

- **그냥 이 플러그인을 쓰고 싶을 뿐이다** (하네스 코드를 직접 고칠 계획은 없다) → **GitHub 주소**로 등록한다. GitHub는 이 플러그인의 코드가 올라가 있는 인터넷 사이트다. 이 방법을 쓰면 내 컴퓨터에 미리 아무 파일도 받아둘 필요가 없다 — Claude Code가 인터넷에서 알아서 가져온다.
...
- **내가 이 하네스(builder-harness) 코드 자체를 고치고 있다** (즉 `~/dev/builder-harness` 폴더에 이 저장소를 이미 내려받아 놓고, 그 안 파일을 직접 수정하는 중이다) → **내 컴퓨터 안의 폴더 경로**로 등록한다. 이렇게 하면 내가 파일을 고칠 때마다 그 수정 내용이 바로 반영된다 — GitHub에 올릴(push) 때까지 기다릴 필요가 없다.
...
**대부분의 사용자는 첫 번째(GitHub 주소)만 쓰면 된다.** 두 번째는 이 하네스를 직접 개발·수정하는 사람 전용이다.
